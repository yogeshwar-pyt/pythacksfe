...
import { LayoutGrid, Waves, Sparkles } from "lucide-react";
...
export function AppSidebar() {
  ...
  const navItems = [
    ...
    {
      href: "/call-processing",
      icon: Waves,
      label: "Process",
    },
    {
      href: "/niner-playground",
      icon: Sparkles,
      label: "Niner",
    },
  ];
  ...
  return (
    <aside className="fixed left-0 top-0 z-50 flex h-screen w-16 flex-col items-center border-r border-slate-800 bg-slate-900 py-4">
      {/* Logo */}
      <Link href="/ao-dashboard" className="mb-6">
        <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-white">
          <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="none" viewBox="0 0 24 24">
            <g fill="#0f172a" clipPath="url(#a)">
            ...
            return (
              <li key={item.href}>
                <Link
                  href={item.href}
                  className={cn(
                    "flex h-11 w-11 flex-col items-center justify-center rounded-lg transition-all",
                    isActive
