...
import { PassEntity, CostData } from '@apis/fetchCostDataByTrailId';
...
const Actions = ({ trailData, expanded, passItem }: ActionProps) => {
    ...
    <Column>
      {expanded ? (
        <Column css={{ gap: '$6i' }}>
          <StyledButton
            ...
            disabled={!passItem?.voucher_link}
            onClick={() => {
              if (!isEmpty(passItem?.voucher_link)) {
      ...
      <Suspense fallback={<FullShimmer />}>
        <HodSidePanel
          ...
          from={'pass'}
          trailCostId={passItem?.trail_cost_id}
          costingId={passItem?.costing_id}
        />
      </Suspense>
    </Column>
  );
