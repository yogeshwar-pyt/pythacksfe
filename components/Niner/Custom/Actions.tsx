...
import { CustomEntity, CostData } from '@apis/fetchCostDataByTrailId';
...
const Actions = ({ trailData, expanded, customItem }: ActionProps) => {
    ...
    <Column>
      {expanded ? (
        <Column css={{ gap: '$6i' }}>
          <StyledButton
            ...
            disabled={!customItem?.voucher_link}
            onClick={() => {
              if (!isEmpty(customItem?.voucher_link)) {
      ...
      <Suspense fallback={<FullShimmer />}>
        <HodSidePanel
          ...
          from={'custom'}
          trailCostId={customItem?.trail_cost_id}
          costingId={customItem?.costing_id}
        />
      </Suspense>
    </Column>
  );
