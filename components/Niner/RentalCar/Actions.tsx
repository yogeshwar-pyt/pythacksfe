...
const Actions = ({ trailData, expanded, rentalCarItem }: ActionProps) => {
    ...
    <Column>
      {expanded ? (
        <Column css={{ gap: '$6i' }}>
          <StyledButton
            ...
            disabled={!rentalCarItem?.voucher_link}
            onClick={() => {
              if (!isEmpty(rentalCarItem?.voucher_link)) {
      ...
      <Suspense fallback={<FullShimmer />}>
        <HodSidePanel
          ...
          from={'rentalcar'}
          trailCostId={rentalCarItem?.trail_cost_id}
          costingId={rentalCarItem?.costing_id}
        />
      </Suspense>
    </Column>
  );
