...
const Actions = ({ trailData, expanded, transferItem }: ActionProps) => {
    ...
    <Column>
      {expanded ? (
        <Column css={{ gap: '$6i' }}>
          <StyledButton
            ...
            disabled={!transferItem?.voucher_link}
            onClick={() => {
              if (!isEmpty(transferItem?.voucher_link)) {
      ...
      <Suspense fallback={<FullShimmer />}>
        <HodSidePanel
          ...
          from={'transfer'}
          trailCostId={transferItem?.trail_cost_id}
          costingId={transferItem?.costing_id}
        />
      </Suspense>
    </Column>
  );
