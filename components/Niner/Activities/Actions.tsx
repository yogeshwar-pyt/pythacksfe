...
const Actions = ({ trailData, expanded, activityItem }: ActionProps) => {
    ...
    <Column>
      {expanded ? (
        <Column css={{ gap: '$6i' }}>
          <StyledButton
            ...
            disabled={!activityItem?.voucher_link}
            onClick={() => {
              if (!isEmpty(activityItem?.voucher_link)) {
      ...
      <Suspense fallback={<FullShimmer />}>
        <HodSidePanel
          ...
          from={'activity'}
          trailCostId={activityItem?.trail_cost_id}
          costingId={activityItem?.costing_id}
        />
      </Suspense>
    </Column>
  );
