...
const Actions = ({ trailData, expanded, hotelItem }: ActionProps) => {
    ...
    <Column>
      {expanded ? (
        <Column css={{ gap: '$6i' }}>
          <StyledButton
            ...
            disabled={!hotelItem?.voucher_link}
            onClick={() => {
              if (!isEmpty(hotelItem?.voucher_link)) {
      ...
      <Suspense fallback={<FullShimmer />}>
        <HodSidePanel
          ...
          from={'stay'}
          trailCostId={hotelItem?.trail_cost_id}
          costingId={hotelItem?.costing_id}
        />
      </Suspense>
    </Column>
  );
