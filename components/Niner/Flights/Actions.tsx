...
import { CostData, FlightEntity } from '@apis/fetchCostDataByTrailId';
...
const Actions = ({ trailData, expanded, flightItem }: ActionProps) => {
    ...
    <Column>
      {expanded ? (
        <Column css={{ gap: '$6i' }}>
          <StyledButton
            ...
            disabled={!flightItem?.voucher_link}
            onClick={() => {
              if (!isEmpty(flightItem?.voucher_link)) {
      ...
      <Suspense fallback={<FullShimmer />}>
        <HodSidePanel
          ...
          from={'flight'}
          trailCostId={flightItem?.trail_cost_id}
          costingId={flightItem?.costing_id}
        />
      </Suspense>
    </Column>
  );
